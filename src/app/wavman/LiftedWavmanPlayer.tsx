...
const LiftedWavmanPlayer: React.FC<{}> = ({}) => {
  ...
  const [randomChars, setRandomChars] = useState<string[]>(
    getHexCharacters(
      randomTrackFeatureFlag ? RANDOM_CHAR_FILTER_AMOUNT : hexChars.length
    )
  );
  ...
  useEffect(() => {
    ...
    if (trackIndex > kind1Events.length - 1 && kind1NowPlaying) {
      console.log(
        `you've listend to all of the tracks that were loaded, please refresh the page to load more :)`
      );
